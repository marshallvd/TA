...
class PenilaianCoreValuesController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            'details' => 'required|array',
            'details.*.id_detail_penilaian_core_values' => 'required|exists:tb_detail_penilaian_core_values,id_detail_penilaian_core_values',
            'details.*.nilai' => 'required|numeric|min:0|max:100',
        ]);
        ...
        try {
            ...
            foreach ($request->details as $detail) {
                $detailPenilaian = DetailPenilaianCoreValues::findOrFail($detail['id_detail_penilaian_core_values']);
