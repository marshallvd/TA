...
document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        ...
        // Coba berbagai cara untuk mendapatkan ID Lamaran
        let idLamaran = 
            wawancaraResponse.data.id_lamaran_pekerjaan || 
            wawancaraResponse.data.lamaran_pekerjaan?.id_lamaran_pekerjaan ||
            wawancaraResponse.data.wawancara?.id_lamaran_pekerjaan ||
            wawancaraResponse.data.wawancara?.lamaran_pekerjaan?.id_lamaran_pekerjaan;

        console.log('ID Lamaran:', idLamaran);

        if (!idLamaran) {
            throw new Error('ID Lamaran tidak ditemukan');
        }

        let lamaranResponse = await fetchData(`lamaran/${idLamaran}`);

        // Dapatkan data dari berbagai kemungkinan struktur
        const pelamar = 
            wawancaraResponse.data.pelamar || 
            wawancaraResponse.data.wawancara?.pelamar || 
            lamaranResponse.data.pelamar;

        const lowongan = 
            wawancaraResponse.data.wawancara?.lamaran_pekerjaan?.lowongan_pekerjaan || 
            lamaranResponse.data.lowongan_pekerjaan;

        const wawancara = 
            wawancaraResponse.data.wawancara || 
            wawancaraResponse.data;

        const lamaran = 
            wawancaraResponse.data.wawancara?.lamaran_pekerjaan || 
            lamaranResponse.data;

        // Isi data ke dalam elemen HTML
        ...
        document.getElementById('lamaranStatus').innerHTML = getBadgeForStatus(lamaran?.status_lamaran || '-');
        document.getElementById('lamaranDate').textContent = formatDate(lamaran?.tanggal_dibuat);
